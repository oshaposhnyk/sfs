...
## Read this first — `.context/` is the source of truth

All project knowledge for agents lives in `.context/`. Read in this order:
...
- Git: never commit/push/merge/reset without explicit owner approval. Never
  `rm -rf` — move to `~/.Trash/`. Ask before touching `db/`, `lib.php`,
  capabilities, auth, cron, Docker.
